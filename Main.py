...
def inscricao():
    ...
    with ((form_inscricao.form("Inscricao"))):
        ...
        if confirma_button:
            if input_email == '':
                st.warning("Informe o E-mail!", icon="⚠")
                st.stop()

            if input_cpf == '':
                st.warning("Informe o CPF!", icon="⚠")
                st.stop()

            if input_nome == '':
                st.warning("Informe o primeiro Nome!", icon="⚠")
                st.stop()
            ...
            if input_dn == '':
                st.warning("Informe sua Data de Nascimento!", icon="⚠")
                st.stop()

            if input_telefone == '':
                st.warning("Informe o número do Celular!", icon="⚠")
                st.stop()

            if input_rua == '':
                st.warning("Informe a Rua!", icon="⚠")
                st.stop()

            if input_bairro == '':
                st.warning("Informe o Bairro!", icon="⚠")
                st.stop()

            if input_cidade == '':
                st.warning("Informe a Cidade e Estado (UF)!", icon="⚠")
                st.stop()

            if resultado_cpf is not None:
                st.warning("CPF Já cadastrado!", icon="⚠")
                st.stop()

            if resultado_email is not None:
                st.warning("E-mail Já cadastrado!", icon="⚠")
                st.stop()

            if not check_aceita:
                st.warning("Necessário aceitar o Termo de Responsabildade!", icon="⚠")
                st.stop()
            ...
            if idmodalidade > 1 and input_participantes == '':
                st.warning("Informe os nomes dos integrantes da Equipe do Desafio 200k", icon="⚠")
                st.stop()

            if idmodalidade > 1 and input_equipe200 == '':
                st.warning("Informe o Nome da Equipe do Desafio 200k", icon="⚠")
                st.stop()
            ...
            try:
                ...
                st.warning("Erro no Banco de Daods, tente novamente, se persistir contate o Administrador do Sistema! {}".format(error), icon="⚠")
                st.stop()
